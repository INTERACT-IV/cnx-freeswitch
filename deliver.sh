...
pkg_names="cnx-fsw-application-curl"
...
user=$(grep "USER" ~/deliver.conf | awk '{print $2}')
repo=$(grep "REPO" ~/deliver.conf | awk '{print $2}')
...
for pkg_name in ${pkg_names}; do
	...
	if [[ "$last_rpm_file" != "" ]]; then
		...
		if [[ "$repo" == "foreman" ]]; then
			...
			scp ${build_dir}/$last_rpm_file $user@$delivery_server:$channel_dir
			ssh ${user}@$delivery_server "createrepo $channel_dir"
			ssh ${user}@$delivery_server "hammer repository synchronize --organization Connectics --product Centos_9_Stream --name Centos_9_IV3US"
			ssh ${user}@$delivery_server "hammer content-view publish --organization Connectics --name IV3US"
			...
			ssh ${user}@$delivery_server "mkdir -p $rpm_architecture"
			scp ${build_dir}/$last_rpm_file $user@$delivery_server:$rpm_architecture
			# Le package est ensuite copié puis signé dans un channel 
			channel_dir=/var/www/html/pub/distro/$delivery_channel
			ssh ${user}@$delivery_server "sudo cp $rpm_architecture/$last_rpm_file $channel_dir"
			ssh ${user}@$delivery_server "sudo rpm --addsign $channel_dir/$last_rpm_file"
			ssh ${user}@$delivery_server "sudo createrepo $channel_dir"
		...
		echo "ERREUR : Pas de package trouvé ${build_dir}/${pkg_name}*.rpm"
		exit 2
	fi
...
for repo in $repos_to_sync; do
	channel_to_sync=$(ssh delivery@$delivery_server "sudo /usr/bin/spacewalk-repo-sync -l | grep centos8-iv3us-qua | cut -d ' ' -f 2")
	ssh delivery@$delivery_server "sudo /usr/bin/spacewalk-repo-sync --channel $channel_to_sync"
done
